...
def check_solution_x():
    ...
    graph = load_mygraph(DataDir, graph_name=graph_name)
    simulator = SimulatorMaxcut(sim_name=graph_name)
...
def check_simulator():
    ...
    simulator = SimulatorMaxcut(graph=graph, device=device)
...
def find_best_num_sims():
    ...
    graph = load_mygraph(DataDir, graph_name=graph_name)
    device = th.device(f'cuda:{gpu_id}' if th.cuda.is_available() and gpu_id >= 0 else 'cpu')
    simulator = SimulatorMaxcut(graph=graph, device=device, if_bidirectional=False)
