"""
Inference script for MIS (Maximum Independent Set) problem using PIGNN (Physics-Inspired Graph Neural Network).

This script implements the inference pipeline for the MIS optimization problem
...
from rlsolver.methods.PIGNN.util import eval_MIS, resolve_device
...
def load_model(model_path: str, device: torch.device) -> PIGNN:
    """
    Load a pre-trained PIGNN model for MIS inference.
...
def save_results(energies: List[float], approximation_ratios: List[float],
    ...
    results_file = os.path.join(output_dir, "mis_inference_results.txt")
    with open(results_file, 'w') as f:
        f.write("MIS Inference Results\n")
        f.write("=" * 50 + "\n")
    ...
    detailed_file = os.path.join(output_dir, "mis_detailed_results.txt")
    ...
    if len(predictions) <= 1000:  # Only save if not too large
        predictions_file = os.path.join(output_dir, "mis_predictions.pt")
...
def run():
    """
    Main inference function for MIS problem.
    ...
    model_path = get_model_load_path(problem="MIS", num_nodes=INFERENCE_NUM_NODES)
