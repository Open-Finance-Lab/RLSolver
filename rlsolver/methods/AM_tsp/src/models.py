...
from RLSolver.rlsolver.methods.AM_tsp.src.layers import GraphEmbedding, AttentionModule, Glimpse, Pointer
...
class AttentionTSP(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Initialize query with learned weights
        query = h_context + self.v_weight_embed(self.init_w)
        ...
        for step in range(self.seq_len):
            ...
            # Point to next node
            probs, _ = self.pointer(glimpsed_query, encoded, mask)
            ...
            # Store results
            log_probs.append(categorical.log_prob(action))
            ...
            selected_h = encoded.gather(1, action_expanded).squeeze(1)
            
            if first_node_h is None:
                first_node_h = selected_h
                
            # Query combines context with path information
...
class TSPSolver(nn.Module):
    ...
    def forward(self, inputs):
        ...
        tour = inputs.gather(1, actions.unsqueeze(2).expand(-1, -1, 2))
