...
def metro_sampling(probs, start_status, max_transfer_time, device=DEVICE):
    num_node = len(probs)
...
def mcpg_sampling_maxcut(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
...
def mcpg_sampling_maxcut_edge(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
    probs = probs.to(torch.device("cpu"))
...
def mcpg_sampling_rcheegercut(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
    probs = probs.to(torch.device("cpu"))
    nvar = data.num_nodes
    edges = data.edge_index
    ...
    graph_probs = start_result.clone()
    # get probs
    raw_samples = metro_sampling(probs, graph_probs, change_times)
...
def mcpg_sampling_ncheegercut(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
    probs = probs.to(torch.device("cpu"))
    nvar = data.num_nodes
    edges = data.edge_index
    nlr_graph, nlc_graph = edges
...
def mcpg_sampling_maxsat(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
...
def mcpg_sampling_qubo(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
    Q = data['Q'] # obj: 1/2 * x^T Q x + b^T x
    nvar = data['nvar']
    raw_samples = start_result.clone()
    # get probs
    raw_samples = metro_sampling(probs, raw_samples, change_times, device)
    samples1 = raw_samples.clone()
    samples2 = samples1 * 2 - 1
    # local search
    for cnt in range(num_ls):
        for index in range(nvar):
            samples2[index] = 0
            res = torch.matmul(Q[index], samples2)
            ind = (res > 0)
            samples2[index] = 2 * ind - 1
    # compute value
    res_sample = torch.matmul(Q, samples2)
    res_sample = torch.sum(torch.mul(samples2, res_sample), dim=0)
    res_sample_reshape = torch.reshape(res_sample, (-1, total_mcmc_num))
    index = torch.argmax(res_sample_reshape, dim=0)
    index = torch.tensor(list(range(total_mcmc_num)), device=device) + index * total_mcmc_num
    max_res = res_sample[index]
    samples = (samples2 + 1) / 2
    return max_res, samples[:, index], raw_samples, -(res_sample - torch.mean(res_sample.float())).to(device)


def mcpg_sampling_qubo_bin(data, start_result, probs, num_ls, change_times, total_mcmc_num, device=DEVICE):
    Q = data['Q'] # obj: 1/2 * x^T Q x + b^T x
    nvar = data['nvar']
    raw_samples = start_result.clone()
    # get probs
