...
class Simpler(torch.nn.Module):
    ...
    def forward(self, device=torch.device(f'cuda:{GPU_ID}' if torch.cuda.is_available() and GPU_ID >= 0 else 'cpu')):
...
def maxcut_dataloader(path, device=torch.device(f'cuda:{GPU_ID}' if torch.cuda.is_available() and GPU_ID >= 0 else 'cpu')):
    with open(path) as f:
        fline = f.readline()
        fline = fline.split()
...
def append_neighbors(data, device=torch.device(f'cuda:{GPU_ID}' if torch.cuda.is_available() and GPU_ID >= 0  else 'cpu')):
    data.neighbors = []
    data.neighbor_edges = []
...
def run():
    ...
    graph_name = "gset_14"
    save_graph_list_to_txt(graph_list=load_graph_list(dataDir=DataDir, graph_name=graph_name), txt_path=path)
