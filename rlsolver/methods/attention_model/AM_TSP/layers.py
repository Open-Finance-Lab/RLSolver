...
class GatedAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        # Pre-LN self-attention with gating
        normed = self.ln1(x)
        batch_size, seq_len, _ = normed.shape
        
        # 计算门控信号
        gate_features = self.gate_proj(normed)  # [batch, seq_len, embed_dim*2]
        gate_signal, value_modulation = torch.split(gate_features, self.embed_dim, dim=-1)
        gate = torch.sigmoid(self.gate_norm(gate_signal))  # [batch, seq_len, embed_dim]
        ...
        # 应用门控机制 - 核心创新点
        # 门控调制注意力输出，并加入值调制信号
        gated_output = gate * attn_output + (1 - gate) * torch.tanh(value_modulation)
